...
public:
    ...
    // 删除好友业务
    void deleteFriend(const TcpConnectionPtr &conn, nlohmann::json &js, Timestamp time);
    ...
    // 退出组群业务
    void exitGroup(const TcpConnectionPtr &conn, nlohmann::json &js, Timestamp time);
    // 组群聊天业务
...
private:
    // 存储消息 id 和其对应的业务处理方法，消息处理器的一个表，写消息 id 对应的处理操作
    std::unordered_map<int, MsgHandler> _msgHandlerMap;

    // 存储在线用户的通信连接，用户的 id， TcpConnectionPtr
