...
EventLoop::EventLoop()
    : looping_(false),
      ...
      poller_(Poller::newDeaultPoller(this)),
      ...
      wakeupChannel_(new Channel(this, wakeupFd_))
{
    ...
    wakeupChannel_->setReadCallback(
        std::bind(&EventLoop::handleRead, this) // 设置 wakeupfd 的事件类型以及发生事件后的回调操作
    );
...
void EventLoop::runInLoop(Functor cb)
{
    ...
    else // 非当前EventLoop线程中执行cb，就需要唤醒EventLoop所在线程执行cb
    {
        queueInLoop(cb);
    }
}
...
void EventLoop::doPendingFunctors()
{
    ...
    {
        std::unique_lock<std::mutex> lock(mutex_);
        functors.swap(pendingFunctor_); // 因为这个函数要一直不断从pendingFunctors_拿1个回调执行拿1个回调执行，然后删掉，在这个时候，如果有很多回调，这里就一直锁着，导致mainloop阻塞住向你下发channel的操作上。造成服务器的时延变长。
    }
